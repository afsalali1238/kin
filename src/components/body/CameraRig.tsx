...
export default function CameraRig({back,zoom,reset,active='',mini=false,onDrag}:{back:boolean;zoom:number;reset:number;active?:string;mini?:boolean;onDrag?:(drag:boolean)=>void}){
 const ref=useRef<Controls>(null);const moving=useRef(true);const {invalidate,camera}=useThree();const angle=useRef(back?Math.PI:0);const target=useRef(new Vector3(0,.9,0));const distance=useRef(3.6);
 // Rest pose frames the full 1.80 m mannequin, crown to feet, at 3.6 m
 // distance on a 35° vertical FOV, centred on the anatomical midpoint.
 useEffect(()=>{const r=!mini?regions.find(r=>r.id===active):null;target.current.set(r?r.focusTarget[0]*.3:0,r?MathUtils.clamp(r.focusTarget[1],.58,1.27):.9,0);distance.current=(r?2.65:3.6)/zoom;angle.current=back?Math.PI:0;moving.current=true;invalidate();},[back,reset,active,zoom,mini,invalidate]);
 ...
 return <OrbitControls ref={ref} target={[0,.9,0]} enablePan={false} enableDamping dampingFactor={.12} minDistance={1.4} maxDistance={5} minPolarAngle={Math.PI/2-.6} maxPolarAngle={Math.PI/2+.6} onStart={()=>{moving.current=false;onDrag?.(true);}} onEnd={()=>onDrag?.(false)} onChange={()=>invalidate()}/>;
}
