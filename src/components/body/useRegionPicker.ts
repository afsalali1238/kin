...
export function useRegionPicker(){
 ...
 useEffect(()=>{const image=new Image();image.src='/models/body-regions.png';image.onload=()=>{const canvas=document.createElement('canvas');canvas.width=image.width;canvas.height=image.height;const c=canvas.getContext('2d');if(c){c.drawImage(image,0,0);pixels.current=c.getImageData(0,0,image.width,image.height);}};return()=>{image.onload=null;pixels.current=null;};},[]);
