...
- Front/back and focus use damped spherical camera transitions. Pins have a surface-normal offset and distance-scaled size. Exactly **one precise point** can be placed (pending → confirm), re-placed by re-tapping, graded on one intensity slider, and removed; **continuous pin dragging and per-surface-normal pin orientation are not implemented**.
...
## Engineering hardening (2026-09-11)

- **Structure**: `KinesioApp.tsx` (≈76 KB god-component) is split into `src/features/{journey,programme,home,session,progress,learn,app}` screens, `src/hooks/useRecovery` (persistence + debounced sync + legacy `v1→v2` single-pin migration via `migrateRecovery`), `src/lib/derive` (deterministic journey derivation) and shared `src/components/ui` primitives. The orchestrator keeps flow state; screens are presentation-only.
- **Tests & CI**: Vitest unit suites cover `match`, `makeProgramme` dosage/phase safety, `redFlags`, `progression`, `traffic`, `variant`, versioned storage migration, the API payload schema and the rate limiter (`npm test`). GitHub Actions runs `typecheck → lint → test → build` on PRs and `main`.
- **API safety**: `PUT/GET /api/recovery` validates against a bounded zod schema (ranges, lengths, array caps, finite 3D coordinates), caps the raw body at 128 KB, and rate-limits per (IP, id) — 30 writes / 60 reads per minute, in-memory best effort. Unknown keys are stripped so future clients stay compatible. Auth on the anonymous UUID remains the known gap; the id must stay unguessable.
- **Accessibility**: `aria-live` on the pin-confirm panel, `role="img"`/`aria-label` on the interactive canvas (mini viewers hidden from AT), `aria-pressed` on toggle chips, 44 px minimum chip targets, and the React-Compiler-era lint rules are now clean without suppressions except documented three.js frame-loop mutations.
